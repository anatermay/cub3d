...
/*   Updated: 2025/11/23 15:04:50 by aternero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../header_files/cub3d.h"

static int	process_ifs(char *line, int index)
{
	int	length;

	length = index;
	while (line[length] && is_space(line[length]) == FALSE)
		length++;
	if (length <= 6)
...
static int	process_tex(t_coord *coord, char *line)
{
	...
	while (is_space(line[index]) == TRUE)
		index++;
	if (process_ifs(line, index) == FALSE)
		return (FALSE);
